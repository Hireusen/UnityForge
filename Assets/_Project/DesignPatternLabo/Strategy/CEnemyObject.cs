...
{
    public class CEnemyObject : MonoBehaviour, IEntity
    {
        #region ─────────────────────────▷ 내부 변수 ◁─────────────────────────
        [Header("능력치")]
        [SerializeField] private int _health = 1000;
        [SerializeField] private int _physical = 0;
        [SerializeField] private int _magical = 0;
        ...
        {
            _health -= damage;
            Debug.Log($"적은 {damage}의 대미지를 받았다! (남은 체력 : {_health})");
            TryDie();
